...
11. 对白要根据上下文推断说话人；同一角色必须使用相同的名字和 voice，保持人物性格和说话风格的一致性。已知角色表：${knownJson}（为 {} 表示尚未识别任何角色，已知角色必须沿用表中的名字和 voice）。
12. text 原文片段本身不得改写、增补或删减。transition 和 text 是分开的：text 保持原样，导演的衔接语写在 transition 中。所有 text 按原文顺序拼接后应完整覆盖输入文本。
13. 确实无法确定说话人的对白，speaker 使用 "${UNKNOWN_SPEAKER_NAME}"，并根据语境为其推断最合适的各项参数。${strictReminder}
